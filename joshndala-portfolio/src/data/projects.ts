...
export const TOP_PROJECTS: Project[] = [
    {
        ...
        longDescription: "The Third Act is a desktop movie journaling application built entirely with JavaFX. It goes beyond a simple watch tracker by prompting users to write four distinct reflections per film: The Summary, The Vibe, The Peak Moment, and Extra Notes. Movies are searched via the TMDb API, pulling in posters and high-res backdrops that are displayed as cinematic gradient-overlay cards. All data is stored locally in a single SQLite file, keeping your journal completely private. A built-in Cinema Tracker monitors progress toward a personal goal of two theater visits per month.",
        achievements: [
            "Designed a cinematic dashboard displaying journal entries as full-backdrop image cards with gradient overlays, built entirely with pure JavaFX code (no FXML)",
            "Integrated the TMDb API to pull movie metadata, posters, and high-resolution backdrops on search",
            "Implemented Letterboxd-style half-star ratings (0–5 in 0.5 increments) for precise film scoring",
            "Built a Cinema Tracker feature to monitor progress toward a monthly theater-going goal",
            "Kept all data completely local and private using a single SQLite file stored on the user's machine"
        ],
    ...
    {
        img: "/image/hylite-studio.png",
        title: "Hylite Studio – Sports Recruitment Auditor",
        desc: "An AI-powered platform that helps sports scouts and recruiters make smarter decisions by cross-referencing player videos, stat sheets, and reports through Gemini 3.0.",
        longDescription: "Hylite Studio is a professional-grade sports recruitment audit tool that accepts a player's full case file — videos, PDFs, stat sheets, images, or any combination — and uses Google's Gemini 3.0 AI to intelligently analyze it. Depending on what's uploaded, it automatically selects the best analysis mode: Audit (cross-referencing report claims against video), Scout (evaluating gameplay from scratch), or Analyst (reviewing stats for red flags). The result is a structured verdict with a 1–10 scout rating, timestamped key moments, strengths and weaknesses, and any red flags.",
        achievements: [
            "Built a dynamic analysis engine that automatically selects the right AI mode (Audit, Scout, or Analyst) based on the uploaded file types",
            "Supported multi-file upload including videos, PDFs, images, and Excel spreadsheets in a single case file",
            "Implemented a playlist-style video player with clickable AI-generated timestamps that auto-switch between videos and seek to key moments",
            "Deployed a high-performance Go backend capable of handling heavy video processing, integrated with the Gemini 3.0 GenAI SDK",
            "Designed a premium dark glassmorphic UI with Vue 3 and Tailwind CSS"
        ],
...
export const MCP_PROJECTS: Project[] = [
    {
        title: "Music Media MCP Server",
        desc: "An MCP server that generates AI-powered music videos from image or video inputs by orchestrating a multi-model pipeline.",
        achievements: [
            "Automates visual analysis using Gemini Vision to prompt Lyria 3 for dynamic soundtrack generation, merging audio and media into an inline HTML video player."
        ],
        technologies: ["Python", "Gemini Vision", "Google Lyria 3", "FFmpeg", "Google Cloud Run"],
        githubLink: "https://github.com/joshndala/music-media-mcp",
    },
    {
        title: "Movie Battle MCP Server",
        desc: "An MCP server that fetches real-world movie data from the OMDb API to generate visual, side-by-side comparison battle cards for film analysis.",
        achievements: [
            "Provides real-time visual analytics comparing Rotten Tomatoes scores and Box Office performance with dynamic, theme-based HTML styling."
        ],
        technologies: ["Python", "OMDb API"],
        githubLink: "https://github.com/joshndala/movie-mcp-server",
    }
];
